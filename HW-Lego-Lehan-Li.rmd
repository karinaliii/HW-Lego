...
lego_sales %>% 
  ...
  head(3)
```
  - #### The first three common first names of purchasers are ‘Jackson', 'Jacob', and 'Joseph'.
...
lego_sales %>% 
  ...
  head(3)
```
  - #### The top three most common themes are 'Star Wars', 'Nexo Knights', and 'Gear'.
...
lego_sales %>% 
  ...
  head(1)
```
  - Among the most common theme of Lego (which is 'Star Wars'), 'The Force Awakens' is the most common subtheme. 
...
```
  ...
  - #### Ages that between 36 and 50 years old has purchuased the highest number of lege sets (313).
...
```
  ...
  - #### Age group 36-50 has spent the most money on Legos.
...
```
  - #### Area code '956' has spent the most money on Legos.
...
```
  ...
  - #### According to the visualization, I can see that 'City' is the most popular theme for age 18 and under; 'Nexo Knights' is the most popular for age 19-25; Star Wars is the most popular for age 26-35, and so does for age 36-50, and 51 and above'.
